...
bool RoutesGetter::IsValidSegment(const nlohmann::json& segment) {
    return (segment.contains(YaJsonWords.kArrivalCodeWord) &&
...
RoutesGetter::RoutesGetter(const ApiMng& api_mng, const ConfigVariables& cfg) {
    const std::string kAddedPath = cfg.date_ + "_" 
...
void RoutesGetter::FillRouteFromSegment(const nlohmann::json& inp_json, const nlohmann::json& segment, 
                                        RouteInfo& route) {
    ...
    route.departure_city = from_city[YaJsonWords.kTittleCodeWord].get<std::string>();
    route.arrival_city = to_city[YaJsonWords.kTittleCodeWord].get<std::string>();
    ...
    if (!segment.contains(YaJsonWords.kDetailsCodeWord)) {
        ...
        route.transport = departure_info[YaJsonWords.kTransportCodeWord].get<std::string>();

        const std::string departure_type = departure_info[YaJsonWords.kStationTypeNameCodeWord];
        const std::string departure_title = departure_info[YaJsonWords.kTittleCodeWord];
        
        route.departure_station = departure_type + " - " + departure_title;

        const std::string arrival_type = arrival_info[YaJsonWords.kStationTypeNameCodeWord];
        const std::string arrival_title = arrival_info[YaJsonWords.kTittleCodeWord];
