...
nlohmann::json ApiMng::GetJson() const {
    ...
    if (connect.status_code != 200) {
        std::cerr << "HTTP request failed with status code: "
                    << connect.status_code << '\n';
...
void ApiMng::SaveKey() const {
    std::fstream api_key_file(kDefaultApiKeyFilePath, std::ios::in
...
std::string ApiMng::Encode() const {
    ...
    for (std::size_t i = 0; i < api_key_.size(); ++i) {
